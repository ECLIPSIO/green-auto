    ...
    return(
        <table className="table table-striped">
            ...
            {tableData.map((s, index) => (
                <tr key={index}>
                    <td>{s['name']}</td>
                    <td>{numberFormatter(s['hit_current'])}</td>
                    <td>{numberFormatter(s['hit_hist'])}</td>
                    <td>{timeFormatter(s['site_time'])}</td>
                    <td>{numberFormatter(Math.round(s['page']*10)/10)}</td>
                    <td>{numberFormatter(Math.round(s['bounce']))}%</td>
                </tr>
            ))}
            </tbody>
        </table>
